...
" @click="addChapter()">新增章节</el-button>
        ...
        <div class="chapter_btn">
          <el-button type="primary" @click="reBack('comic','m')">返回角色页</el-button>
        </div>
      </div>
    </div>
...
const roleManage = () => import("./ruleManage");
...
export default {
  ...
  data() {
    return {
      ...
      //表格
      tableRule: [
        { label: "角色id", prop: "id", width: "150", hide: true },
        { label: "角色名", prop: "roleName", width: "auto" },
        { label: "添加时间", prop: "ad", hide: true }
      ],
      //表格按钮
      tableBtn: {
        fixed: "right", //固定右侧
        ...
        btnGroup: [
          {
            ...
            method: (data, idx) => {
              ...
              // this.$api.delRole(Qs.stringify({ id: data.id })).then(res => {
              //   console.log(res);
              // });
            }
          }
  ...
  methods: {
    ...
    //删除角色
    delRole(id) {
      this.$api.delRole(Qs.stringify({ id })).then(res => {
        if (res == 200) {
          this.$router.go(0);
        }
      });
    ...
    submitForm(formName) {
      ...
      if (state == "view") {
        ...
        //添加用户
        this.$api.addRole(post).then(res => {
          if (res == 200) {
            this.$router.go(0);
          }
        });
